# FRED S&P 500 Adapter Contract

> Binding for any code path that fetches or parses S&P 500 daily index
> observations from FRED. Lower-priority instructions (chat, scripts,
> LLM completions) cannot override this document. If in conflict, this
> document wins.

This document defines the FRED public S&P 500 adapter contract. It
specifies:
...
The FRED S&P 500 adapter is the **second approved public, no-auth
network adapter** in NikkeiMicroScope. It is implemented in
`nms/data/fred_sp500.py` and is documented in full by this document.
The parent contract view is in
[`docs/data-adapter-contract.md`](data-adapter-contract.md) §8.2.
...
* Nasdaq-100, SOX, FX, or Nikkei data.
...
| Target field | Source |
| --- | --- |
| `sp500` | SP500 value at the selected date |
| `sp500_change_pct` | `((SP500_today / SP500_yesterday) - 1.0) * 100.0` |

All other `us_equities` fields and all other `MarketContext`
sections come from the base adapter and are left unchanged.
...
1. The latest SP500 observation at or before `session_date` is
   selected. If none exists, `FredSP500AdapterError` is raised.
2. The previous SP500 observation (strictly before the selected
   date) is used to compute `sp500_change_pct`. **If none
   exists, `FredSP500AdapterError` is raised.** A missing
...
3. If the previous SP500 value is non-positive (≤ 0),
   `FredSP500AdapterError` is raised. A non-positive previous value
...
`FredSP500AdapterError` is raised.
...
* Negative tests for:
  * Missing previous SP500 → `FredSP500AdapterError`.
  * Non-positive previous SP500 → `FredSP500AdapterError`.
  * Malformed CSV (date, numeric, missing column, empty) →
    `FredSP500AdapterError`.
* A test that only `us_equities.sp500` and
  `us_equities.sp500_change_pct` are overlaid (all other fields
  come from the base adapter unchanged).
...
* Any predictive accuracy of any score that consumes the overlaid
  fields.
* That the SP500 endpoint is reachable at any particular moment.
  The dry-run uses mocked CSVs.
* That the adapter reads Nasdaq-100, SOX, FX, or Nikkei data
  (those are out of scope for this PR).
* That missing data is silently treated as a neutral contribution.
  A missing required input (e.g. no previous SP500 observation)
  causes `FredSP500AdapterError` to be raised.
